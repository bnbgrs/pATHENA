...
def _app() -> QApplication:
    return create_application(["pathena-v3-comfyui-test"])
...
def test_v3_comfyui_is_shell_hosted_and_restores_selected_route() -> None:
    app = _app()
    ...
    try:
        ...
        assert controller._shell_generation == "v3"
        assert window.navigation.currentRow() == 0
        assert window.pages.currentIndex() == 0

        title = window.findChild(QLabel, "v3PageTitle")
        hint = window.findChild(QLabel, "v3PageHint")
        workspace = window.findChild(QWidget, "v3Workspace")
        ...
        assert controller.dialog.isHidden()
        assert shell._pallas_button.property("active") is True
        assert title.text() == "PALLAS"
        assert hint.text() == "A living semantic field grounded in real local state."
