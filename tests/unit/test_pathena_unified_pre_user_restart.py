...
class _Provider:
    ...
    def stream_chat(
        ...
        *,
        ...
        self.calls += 1
        yield "Recovered after pre-user crash [MODEL-PRIOR]"
...
def test_pre_user_plan_survives_rolled_back_user_transaction_and_retry(
    tmp_path: Path,
) -> None:
    ...
    content = "Recover this exact pre-user Unified send."
    ...
    try:
        ...
        database.connection.execute(
            ...
            BEGIN
                SELECT RAISE(ABORT, 'synthetic pre-user transaction crash');
            END
            """
        )
        service = _service(
        ...
        with pytest.raises(sqlite3.IntegrityError, match="synthetic pre-user"):
            _send(
                service,
                chat_id=chat_id,
                operation_id=operation_id,
        ...
        first_plan = UnifiedSendPlanRepository(database).load(
            operation_id,
            fingerprint=fingerprint,
        )
        assert first_plan is not None
        assert first_plan.operation_id == operation_id
        assert first_plan.chat_id == chat_id
        ...
        first_plan_created_at = first_plan.created_at_us
        ...
        assert final_plan.payload_sha256 == first_plan_sha
        assert final_plan.created_at_us == first_plan_created_at
        assert GroundedSendRecovery(database).inspect(
            operation_id=operation_id,
            chat_id=chat_id,
            fingerprint=fingerprint,
