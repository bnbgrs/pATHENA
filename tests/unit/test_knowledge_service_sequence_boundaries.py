...
def test_knowledge_promotion_rejects_non_integer_sequence_before_chat_load(
    sequence_no: object,
) -> None:
    chat = _ChatStub()
    service = KnowledgeService(object(), chat)  # type: ignore[arg-type]
    ...
    with pytest.raises(ChatMessageSequenceError, match="integer of at least 1"):
        service.promote_chat_message(
            ...
            knowledge_kind=KnowledgeKind.NOTE,
        )
