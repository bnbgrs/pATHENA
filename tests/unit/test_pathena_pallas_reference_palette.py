...
def test_pallas_v3_semantics_keep_meaningful_workspace_colors() -> None:
    ...
    # Conflict and uncertainty must remain visually distinguishable from
    # trusted/source semantics even when focus and claims share the accent.
    assert _hex(PallasNodeKind.CONFLICT) != _hex(PallasNodeKind.SOURCE)
    assert _hex(PallasNodeKind.UNCERTAIN) != _hex(PallasNodeKind.SOURCE)
