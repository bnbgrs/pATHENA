...
def test_pallas_canvas_uses_v3_canvas() -> None:
    assert _CANVAS.name().casefold() == V3_BG.casefold()


def test_pallas_v3_semantics_keep_distinct_workspace_colors() -> None:
    ...
    assert _hex(PallasNodeKind.KNOWLEDGE) == "#d9d1b8"
    assert _hex(PallasNodeKind.MEMORY) == "#a8c5a2"
    ...
    semantic_colors = {
        _hex(PallasNodeKind.SOURCE),
        _hex(PallasNodeKind.CLAIM),
        _hex(PallasNodeKind.KNOWLEDGE),
        _hex(PallasNodeKind.MEMORY),
        _hex(PallasNodeKind.HYPOTHESIS),
        _hex(PallasNodeKind.CONFLICT),
    }
    assert len(semantic_colors) == 6
