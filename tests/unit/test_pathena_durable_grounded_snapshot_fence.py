...
def test_canonical_drift_inside_boundary_is_fenced_before_provider(tmp_path: Path) -> None:
    ...
    try:
        ...
        drift_chat_id = chats.create_chat(actor_id=user)

        def mutate_canonical_state() -> None:
            ChatService(chats).add_user_message(
                chat_id=drift_chat_id,
                content="concurrent canonical commit",
            )
        ...
        with pytest.raises(
            ...
            generation.send_context_package(
                ...
                on_before_provider_call=mutate_canonical_state,
            )
        ...
        resumable_run = ModelRunRepository(database).load_run(run_id)
        assert resumable_run.status == "running"
        assert resumable_run.finished_at_us is None
        assert resumable_run.error_detail is None
