...
import athena.source.import_intake as import_intake_module
...
def _service(
    ...
    *,
    archive: bool = True,
) -> tuple[ImportIntakeService, _FakeSources]:
...
def _pretend_link(monkeypatch: pytest.MonkeyPatch, link: Path) -> None:
    original = import_intake_module._is_link_or_junction  # noqa: SLF001

    def fake_is_link_or_junction(path: Path) -> bool:
        return path == link or original(path)

    monkeypatch.setattr(
        import_intake_module,
        "_is_link_or_junction",
        fake_is_link_or_junction,
    )


def _redirect_resolve(
    ...
    def fake_resolve(self: Path, strict: bool = False) -> Path:
        if self == source:
            return original(target, strict=strict)
...
def test_recursive_preflight_is_deterministic_and_reports_system_metadata(
    ...
    filtered = [
        ...
        if issue.code == "filtered_system_metadata"
    ]
    assert {issue.path.name for issue in filtered if issue.path is not None} == {
        ".DS_Store",
        ".git",
    }
...
def test_default_symlink_policy_does_not_follow_without_platform_symlink(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "input"
    ...
    _pretend_link(monkeypatch, link)
    service, _sources = _service(tmp_path)
...
def test_follow_policy_rejects_link_outside_selected_root_without_platform_symlink(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "input"
    ...
    link.mkdir()
    _pretend_link(monkeypatch, link)
    _redirect_resolve(monkeypatch, link, outside)
    service, _sources = _service(tmp_path)
    ...
    assert any(
        issue.code == "link_outside_selected_root"
        for issue in preflight.issues
    )
    assert not preflight.candidates


def test_follow_policy_detects_directory_cycle_without_platform_symlink(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "input"
    ...
    cycle = child / "back"
    cycle.mkdir()
    _pretend_link(monkeypatch, cycle)
    _redirect_resolve(monkeypatch, cycle, root)
    service, _sources = _service(tmp_path)
...
def test_preflight_enforces_size_and_reports_expected_count(tmp_path: Path) -> None:
    ...
    assert preflight.blocked
    assert any(
        issue.code == "max_file_size_exceeded" for issue in preflight.issues
    )
    assert any(
        issue.code == "expected_count_mismatch" for issue in preflight.issues
    )
