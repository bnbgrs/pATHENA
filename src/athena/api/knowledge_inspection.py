"""Transport-neutral API adapter for canonical Claim inspection and contradiction review."""
...
class ClaimInspectionBoundary(Protocol):
    """Minimal domain boundary required by the client-facing inspection adapter."""
...
class KnowledgeInspectionApiService:
    """Expose Claim inspection as stable DTOs without leaking repositories."""

    def __init__(
        self,
        *,
        inspection: ClaimInspectionBoundary,
    ...
    def list_claims(self, *, limit: int = 100) -> tuple[CanonicalClaimResponse, ...]:
        snapshots = self._inspection.list_claims(limit=limit)
        return tuple(
            _claim_response(self._inspection.load_claim(snapshot.claim_id))
            for snapshot in snapshots
        )

    def load_claim(self, claim_id: str) -> CanonicalClaimResponse:
        return _claim_response(self._inspection.load_claim(uuid.UUID(claim_id)))
    ...
    def list_pending_contradictions(
        ...
        *,
        ...
        return tuple(
            _contradiction_response(details)
        ...
        *,
        ...
        return _contradiction_response(
            self._inspection.resolve_contradiction_review(
                uuid.UUID(review_id),
                actor_id=self._actor_id_provider(),
                decision=parsed_decision,
            )
        )


def _claim_revision_response(
...
def _claim_response(details: ClaimDetails) -> CanonicalClaimResponse:
    return CanonicalClaimResponse(
        claim_id=str(details.snapshot.claim_id),
        lifecycle_state=details.snapshot.lifecycle_state,
        revision=_claim_revision_response(details.snapshot.revision),
        provenance_inputs=tuple(
            ...
            for item in details.provenance_inputs
        ),
        evidence=tuple(
            ClaimEvidenceResponse(
                ...
                evidence_entity_id=(
                    None
                    if item.evidence_entity_id is None
                    else str(item.evidence_entity_id)
                ),
                evidence_revision_id=(
                    None
                    if item.evidence_revision_id is None
                    else str(item.evidence_revision_id)
                ),
            )
...
def _contradiction_response(
    ...
    return ContradictionReviewResponse(
        ...
        created_at_us=review.created_at_us,
        resolved_at_us=review.resolved_at_us,
        processing_run_id=str(review.processing_run_id),
        model_signature_id=str(review.model_signature_id),
        ...
        decision_actor_id=(
            None if review.decision_actor_id is None else str(review.decision_actor_id)
        ),
