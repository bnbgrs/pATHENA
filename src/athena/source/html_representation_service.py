"""Use cases for retained native HTML SourceRepresentations."""
...
from athena.source.html_representation_store import (
    HtmlNativeTextRepresentationStore,
...
class HtmlRepresentationBuildResult:
    result: TextRepresentationResult
    processing_run: ProcessingRun
    structures: tuple[SourceRepresentationStructureRecord, ...]


class SourceHtmlRepresentationService:
    """Create retained cleaned HTML text plus durable DOM-derived structure."""
    ...
    def __init__(
        ...
        *,
        ...
        representation_store: HtmlNativeTextRepresentationStore,
    ...
    def build(
        ...
        *,
        ...
        prepared: PreparedHtmlTextRepresentation | None = None
        try:
    ...
    def verify_structure_map(
        ...
        if representation.parser_id != _PARSER_ID:
            raise ValueError("HTML structure verification requires native HTML representation.")
        ...
        if not structures:
            raise RuntimeError("Native HTML representation is missing its retained structure map.")
        if tuple(item.structure_index for item in structures) != tuple(range(len(structures))):
            raise RuntimeError("HTML structure map indexes are not contiguous from zero.")
        known_ids: set[uuid.UUID] = set()
        for item in structures:
            if item.parent_structure_id is not None and item.parent_structure_id not in known_ids:
                raise RuntimeError("HTML structure map contains an unknown or forward parent.")
            if not 0 <= item.start_offset <= item.end_offset <= len(text):
                raise RuntimeError("HTML structure range is outside retained text.")
            actual = hashlib.sha256(
                text[item.start_offset : item.end_offset].encode("utf-8")
            ).digest()
            if actual != item.content_hash:
                raise RuntimeError("HTML structure text hash verification failed.")
            try:
                metadata = json.loads(item.metadata_json)
            except json.JSONDecodeError as exc:
                raise RuntimeError("HTML structure metadata is invalid JSON.") from exc
            if not isinstance(metadata, dict):
                raise RuntimeError("HTML structure metadata must be a JSON object.")
            known_ids.add(item.structure_id)
        return structures


def _require_supported_html_source(source: SourceRecord) -> None:
    suffix = Path(source.original_name or "").suffix.lower()
    if source.mime_type in _HTML_MIME_TYPES:
        return
    if suffix in _HTML_SUFFIXES and source.mime_type in _TEXTISH_MIME_TYPES:
        return
    raise UnsupportedHtmlSourceError(
        "VS6 Step 3 native HTML extraction supports HTML snapshot Sources only."
    )
