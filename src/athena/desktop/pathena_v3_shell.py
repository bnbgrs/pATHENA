...
class PathenaV3ShellController(QObject):
    ...
    def set_inspector_available(self, available: bool) -> None:
        """Expose Evidence only when Chat has real context to inspect."""
        self._inspector_available = bool(available)
        is_chat = (
            self._window.navigation.currentRow() == 0
            and not bool(self._window.property("pathenaPallasShellOpen"))
        )
    ...
    def finalize(self) -> None:
        ...
        if self._window.chat_selector.count() == 0:
            self._window.chat_selector.addItem("No conversation selected", None)
        if self._window.model_selector.count() == 0:
            self._window.model_selector.addItem("Waiting for local model…", None)
