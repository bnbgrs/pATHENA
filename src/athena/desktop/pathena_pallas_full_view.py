...
def _apply_reference_renderer_palette(
    ...
    """Align the renderer-only PALLAS palette with the opened reference family.

    The semantic graph, node classes, layout and selection contracts stay untouched.
    ``pathena_pallas_field`` keeps its renderer colors as module presentation constants,
    so updating those values before creating the full workspace is sufficient and
    avoids duplicating graph logic in this shell controller.
    """
