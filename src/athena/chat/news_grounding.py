"""Grounded chat over durable Event-eligible ATHENA News events."""
...
@dataclass(frozen=True, slots=True)
class NewsGroundedChatResult:
    generation: ChatGenerationResult
    context: NewsEventContextBundle
    context_package: ContextPackage
    processing_run: ProcessingRun
    budget: NewsContextBudgetReport


class NewsGroundedChatService:
    ...
    def __init__(
        ...
        *,
        ...
        context_builder: NewsEventContextBuilderService,
    ...
    def send_message(
        ...
        *,
        ...
        sections = (
            ContextSection(
                name="news_event_context",
                role="system",
                content=system_text,
                included_ref_ids=tuple(
                    item.ref_id
                    for item in news_refs
                ),
            ),
            *prior_sections,
            ContextSection(
        ...
        budget_report = NewsContextBudgetReport(
            effective_context_limit=context_limit,
            estimated_input_tokens=estimated_input_tokens,
            context_tokens=context.estimated_tokens,
            output_reserve=output_reserve,
        ...
        *,
        ...
        evidence_refs = tuple(
            GroundingEvidenceRef(
                ...
                news_research_result_id=(
                    item.event.research_result_id
                ),
                news_finding_ordinal=(
                    item.event.finding_ordinal
                ),
                news_finding_hash=(
                    item.event.finding_hash
                ),
                news_source_ids=(
                    item.event.source_ids
                ),
            )
            for item in context.items
        )
