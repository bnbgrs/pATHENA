...
- UI exact `6bc46a2464344d56ca00461df30ba4a619437498`: Core Focused `34714819122 = FAILURE`; canonical `34714819100 = FAILURE`. Core Focused Ruff passes, but its pytest selection contains only two PySide6-dependent UI modules that are skipped under the Core-only environment, exposing `ERR-0046`. Canonical is independently healthy except Ruff: specification validator, mypy, full pytest, Local Install, Linux storage regressions and Windows path safety pass; full pytest is `5000 passed, 17 skipped`. The sole canonical Ruff `I001` is `tests/unit/test_core_focused_candidate_workflow.py`, inherited from Develop parent `522a010...` and separately corrected by current Develop `54c99028...`; no UI product root cause is opened from canonical.
...
- Exact focused artifact: Ruff is `All checks passed!`; focused pytest selects `tests/unit/test_pathena_comfyui_shell.py` and `tests/unit/test_pathena_pallas_full_view.py`, and both modules are skipped at collection because `PySide6` is absent. The run then fails at `Enforce focused candidate outcomes`.
...
- Canonical cross-check on the same UI SHA does not expose a UI runtime defect: full pytest is `5000 passed, 17 skipped`; all canonical jobs except Ruff are green. The single canonical Ruff defect is inherited `tests/unit/test_core_focused_candidate_workflow.py` from Develop parent and is not part of this root cause.
