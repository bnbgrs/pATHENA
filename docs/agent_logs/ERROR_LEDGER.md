...
- Error worker entered this run at `postmerge/errors@3f7f5e35b2248688de4203c1f072e8a9cda92dbc`; current pre-ledger-update worker head was `a9f5d1783fe99d95af954eca8f66bb0e9e3cedee`.
...
- `postmerge/errors@a9f5d1783fe99d95af954eca8f66bb0e9e3cedee` had zero workflow runs before this mutation.
...
- Exact downloaded Storage-focused diagnostics: Ruff passes; Storage mypy passes (`Success: no issues found in 35 source files`); focused pytest fails `2 failed, 29 passed`.
...
- Root-cause cluster is one bounded Backend-owned change: the implementation cannot distinguish legitimate new sidecar publication from foreign/replaced existing sidecars, while its positive test currently does not construct the intended transition.
...
- Ownership remains Spec/Core; Errors does not parallel-edit the owner-held Core slice.
...
- Status: `OPEN`.
- Current exact reproducer: Core Focused Candidate `34709115243` on `postmerge/ui@1c6c3475945c7ee0ba4d7514b81dd4d444d843e6`.
- Exact downloaded diagnostics show this is not an independent UI product failure. `ruff.txt` contains only `E902 No such file or directory` for `src/athena/knowledge/orphan_knowledge.py` and `tests/unit/test_orphan_knowledge.py`; `pytest.txt` says `ERROR: file or directory not found: tests/unit/test_orphan_knowledge.py` and no tests run.
- Root cause is in `.github/workflows/core-focused-candidate.yml`: changed-file/test selection uses `git diff --name-only BASE CANDIDATE` without a deletion-excluding `--diff-filter`. The UI PR lineage deletes both orphan-knowledge files relative to its base, so the workflow selects paths that do not exist at the exact candidate SHA and feeds them to Ruff/Pytest.
- This is a harness-selection defect, not evidence that deleted orphan-knowledge product code itself is broken. Do not assign the failure to UI or duplicate `ERR-0042`.
- Minimal repair: add an appropriate deletion-excluding diff filter (for example `--diff-filter=ACMR`) consistently to Core changed-Python selection, changed focused-test selection, and Ruff remediation selection, preserving rename/modify/add behavior and all enforcement. Then verify against a candidate containing deleted Core/test paths plus a normal changed-file candidate.
- Errors attempted no unsafe broad workflow creation: `.github/workflows/core-focused-candidate.yml` is absent on the current divergent `postmerge/errors` lineage, so a surgical in-place harness patch is not available there without importing unrelated workflow history. Ownership is handed to the current workflow/integration lineage rather than creating a duplicate workflow file on Errors.
