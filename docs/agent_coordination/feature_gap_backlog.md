...
- **Verification:** 2026-08-23; production wiring was revalidated again: `StorageBootstrapService.start()` binds `DiskPressureController.assert_noncritical_write_allowed` through `SQLiteDatabase.configure_noncritical_write_gate()` before database startup, and `SQLiteDatabase.write_transaction()` invokes that gate before `BEGIN IMMEDIATE`. Dedicated pressure-database regressions also cover ordinary-write blocking and explicit recovery writes. No test execution was performed by this scout, so status remains IMPLEMENTED rather than VERIFIED.

### FG-016 — Verify safety-critical SQLite PRAGMAs after applying them
- **Source:** Beta 03 sections 30–40, especially section 30 (`foreign_keys` readback), section 33 (configurable `busy_timeout`), section 37 (`trusted_schema` readback) and sections 39–40 (WAL configuration/monitoring).
- **Ownership / Priority / Status:** BACKEND · P1 · READY
- **Evidence / code paths:** `src/athena/storage/schema.py::_configure_connection()` sets `foreign_keys = ON`, `synchronous = FULL`, `busy_timeout = 5000`, `secure_delete = ON`, `read_uncommitted = OFF`, `wal_autocheckpoint = 1000` and `trusted_schema = OFF`. It explicitly reads back and fail-closes only `journal_mode = WAL`.
- **Current state:** the live schema bootstrap matches most normative PRAGMA values, but it does not read back the two safety-critical values that Beta explicitly requires to be verified (`foreign_keys`, `trusted_schema`). `busy_timeout` is also fixed at 5000 rather than exposed as the spec's configurable runtime value. A silent pragma mismatch therefore has no central compatibility failure for those settings.
- **Desired state:** after applying the connection policy, read back and validate at minimum `foreign_keys` and supported `trusted_schema`, fail closed on mismatch, and define a bounded configurable busy-timeout source without weakening the 5000 ms v1 floor. Add focused regressions that exercise the readback/fail-closed contract. Treat WAL-size monitoring/checkpoint orchestration as a separate B03 follow-up rather than overloading this slice.
- **Dependencies:** none beyond the existing SQLite connection bootstrap. Do not couple this to the Alembic-vs-custom decision in FG-013.
- **Verification:** static current-remote trace on 2026-08-23 against Beta 03 and `schema.py`; no test execution claimed.
...
- FG-016 is a narrow connection-safety handoff: verify applied PRAGMAs and make the bounded busy timeout configurable; keep WAL monitoring/checkpoint policy as a separate scan target.
