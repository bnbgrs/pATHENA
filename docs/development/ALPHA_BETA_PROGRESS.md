...
| Persistent desktop system tray | Alpha Desktop/UI contract + UI-GAP-0005 | `VERIFIED` | Exact UI product/test head `acc156a8538e83ffec4e3eba4b9bef3e9c2fdb37` passed canonical Quality `33814651800`. Integrator independently reviewed and integrated tray controller, focused tests and SystemWorkspace single-install wiring. Unsupported operations remain visible but disabled rather than fabricating success. | UI-GAP-0006 runtime-state visibility remains separate. |
| Tray runtime-state visibility | Alpha Desktop/UI contract + UI-GAP-0006 | `IMPLEMENTED_PENDING_VERIFY` | Earlier canonical `33818773088` completed `cancelled`, not PASS; current UI worker contains a later mypy-focused correction requiring fresh exact verification. | Integrator waits for new exact green evidence. |
