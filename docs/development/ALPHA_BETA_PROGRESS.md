...
| Canonical post-merge error state | Error ledger | `VERIFIED` | Error worker confirms `ERR-0001` through `ERR-0007` FIXED; repaired-lineage canonical Quality `33838658964` is green and source-types canonical Quality `33840621670` introduced no new current-lineage error. | Error resumes fresh scanning; allocate `ERR-0008` only on exact reproducible primary failure. |
