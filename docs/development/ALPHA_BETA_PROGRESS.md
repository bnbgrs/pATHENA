...
| Settings unsaved selected-model defaults freshness | UI-GAP-0015 | `IMPLEMENTED_PENDING_VERIFY` | Current UI handoff reports this gap verified on the worker lineage; independent shared-Develop baseline/diff review remains outstanding. | Integrator may consume in a later single bounded run. |
| Canonical post-merge error state | Error ledger | `PARTIAL` | `ERR-0001` through `ERR-0008` are fixed. `ERR-0009` is `FIXED_PENDING_VERIFY`: product remaining-budget hardening is retained and the harness-only correction still requires exact green descendant evidence. | Backend/Error supply exact green evidence before closure. |
