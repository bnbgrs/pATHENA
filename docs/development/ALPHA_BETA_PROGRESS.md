...
| Canonical post-merge error state | Error ledger | `VERIFIED` | Error worker handoff now reports `ERR-0001` through `ERR-0005` closed; no Error-owned product mutation is accepted in this run. | Error resumes fresh exact-lineage regression scanning. |
