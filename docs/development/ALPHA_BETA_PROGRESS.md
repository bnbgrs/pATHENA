...
| Canonical post-merge error state | Error ledger | `IMPLEMENTED_PENDING_VERIFY` | `ERR-0001` through `ERR-0007` remain FIXED. `ERR-0008` is IN_PROGRESS: Error evidence identifies the remaining red Settings test as a stale loopback-only accessibility expectation, not a product runtime defect; exact UI Quality is still red, so no closure or integration is claimed. | Error/UI must land the minimal substantive expectation correction and obtain fresh exact-green focused/full/canonical verification. |
