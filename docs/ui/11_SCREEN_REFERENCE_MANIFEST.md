...
|---|---|---|---|---|---|
| 01 | Workspace / Chat | `AVAILABLE_OPENED` — `pATHENA: Dunkles KI-Dashboard mit Wissenspanel.png` | Quiet central workspace; chat as work document; large composer; contextual evidence/activity inspector; narrow left-owned primary navigation | `IMPLEMENTED_PENDING_VERIFY` — real composer 88 px; prompt 44 px; Sources 36 px; send target 44×44 outer geometry, with 42 px QSS content-box plus inherited 1 px border per side | `c830b96a12d25914c52a0abc7749a6724b19cfae` + current UI candidate pending exact-SHA Quality |
